...
//            Eigen::VectorXd branch_ratio = Eigen::VectorXd::Constant(nb_line, 1.);
//            fillYbusBranch(branch_r, branch_x, branch_c, branch_ratio, branch_from_id, branch_to_id);
            fillYbusBranch(branch_r, branch_x, branch_c, branch_from_id, branch_to_id);
                        ...
                        )
        {
            ...
            **/
            ...
            for(int trafo_id =0; trafo_id < nb_trafo; ++trafo_id){
                // compute from / to
                int hv_id = trafo_hv_id(trafo_id);
                int lv_id = trafo_lv_id(trafo_id);

                cdouble z = trafo_r(trafo_id) + my_i * trafo_x(trafo_id);
                cdouble y = 1.0 / z;
                double r = ratio(trafo_id);

                // add the b, why not :-/
                cdouble h = trafo_b(trafo_id); // yes it's the correct one
                h = my_i * 0.5 * h;
//                h = 0.5 * h;

                //TODO parrallel powerlines! -> compute this at the end, when all is known and stored, for all trafo
                // and powerlines !!!

                // fill non diagonal coefficient
                cdouble tmp = y / r;
                Ybus_.insert(hv_id, lv_id) = -tmp ;
                Ybus_.insert(lv_id, hv_id) = -tmp;
    ...
    protected:
        ...
        void fillYbusBranch(const Eigen::VectorXd & branch_r,
                            const Eigen::VectorXd & branch_x,
                            const Eigen::VectorXcd & branch_b,
                            const Eigen::VectorXi & branch_from_id,
                            const Eigen::VectorXi & branch_to_id)
        {
            /**
            NOTHING IN INPUT IS PER UNIT
            DOES NOT WORK FOR TRAFO
            **/
            ...
            Eigen::VectorXcd branch_h = Eigen::VectorXcd::Constant(nb_line, 2.0 * f_hz_ * M_PI * 1e-9);
            branch_h.array() *= branch_b.array();
...
//            Eigen::VectorXcd branch_b = branch_cc.array() - 1.0i * branch_gg.array();
            ...
            for(int line_id =0; line_id < nb_line; ++line_id){
                ...
                cdouble z = branch_r(line_id) + my_i * branch_x(line_id);

//                std::cout << " from " << from_id << " to " << to_id << " x " << branch_x(line_id) <<std::endl;
                cdouble y = 1.0 / z;  // y = 1/((r + 1j * x) / baseR)

                // add the b, why not :-/
                cdouble h = branch_h(line_id); // yes it's the correct one
                h = my_i * 0.5 * h;
