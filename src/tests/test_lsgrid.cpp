...
// ---------------------------------------------------------------------------
// unset_changes() / solver-cache consistency
// ---------------------------------------------------------------------------
// `unset_changes()` asserts "the cached solver-side data matches the grid", and
// it does so for BOTH solver families at once. Nothing used to check that
// assertion, so it could be made about a family whose data had never been built
// -- the next powerflow of that family then took the "nothing to rebuild" path
// with an empty id map / Ybus / Sbus and indexed them with bus ids in the
// hundreds. Release wheels are -O3 -DNDEBUG, so that was a segfault, not an
// error: every SECTION below crashed the process before the cache-consistency
// guard at the top of `_pre_process_solver_impl`. They run under valgrind/ASan
// in the cpp_unit_tests workflow, which is what proves the reuse decision is
// made before anything is indexed.
//
// The rule the guard enforces: a wrong "nothing changed" may cost time (a
// rebuild that was not strictly needed), never memory safety.
TEST_CASE("unset_changes never makes a powerflow reuse data that was never built", "[LSGrid][unset_changes]")
{
    SECTION("on a grid that was never solved at all") {
        LSGrid grid = make_three_bus_grid();
        grid.unset_changes();  // nothing has been built yet -- the claim is simply false

        const CplxVect V = solve_ac(grid);
        REQUIRE(V.size() == 3);

        LSGrid fresh = make_three_bus_grid();
        const CplxVect V_fresh = solve_ac(fresh);
        REQUIRE(V_fresh.size() == 3);
        CHECK((V - V_fresh).norm() < 1e-10);
    }

    SECTION("AC after a DC solve: the AC family has nothing cached") {
        LSGrid grid = make_three_bus_grid();
        REQUIRE(grid.dc_pf(flat_start(grid), 1, 1e-8).size() == 3);
        grid.unset_changes();  // clears the AC flags too, though no AC solve ever ran

        const CplxVect V = solve_ac(grid);
        REQUIRE(V.size() == 3);

        LSGrid fresh = make_three_bus_grid();
        CHECK((V - solve_ac(fresh)).norm() < 1e-10);
    }

    SECTION("DC after an AC solve: the DC family has nothing cached") {
        LSGrid grid = make_three_bus_grid(0.);
        REQUIRE(solve_ac(grid).size() == 3);
        grid.unset_changes();  // clears the DC flags too, though no DC solve ever ran

        const CplxVect V = grid.dc_pf(flat_start(grid), 1, 1e-8);
        REQUIRE(V.size() == 3);

        // r = 0, 0.5 pu through x = 0.1 pu per line: 0 / -0.05 / -0.10 rad
        CHECK(std::arg(V(0)) == Approx(0.).margin(1e-10));
        CHECK(std::arg(V(1)) == Approx(-0.05).margin(1e-10));
        CHECK(std::arg(V(2)) == Approx(-0.10).margin(1e-10));
    }

    SECTION("alternating AC / DC solves, each followed by unset_changes") {
        // `slack_weights_`, `bus_pv_` and `bus_pq_` are SHARED by the two families,
        // so even a family whose own id map / matrix is up to date must rebuild
        // them when the other family wrote them last.
        LSGrid grid = make_three_bus_grid(0.);
        LSGrid ref_ac = make_three_bus_grid(0.);
        LSGrid ref_dc = make_three_bus_grid(0.);
        const CplxVect V_ac_ref = solve_ac(ref_ac);
        const CplxVect V_dc_ref = ref_dc.dc_pf(flat_start(ref_dc), 1, 1e-8);
        REQUIRE(V_ac_ref.size() == 3);
        REQUIRE(V_dc_ref.size() == 3);

        for(int round = 0; round < 3; ++round){
            const CplxVect V_ac = solve_ac(grid);
            REQUIRE(V_ac.size() == 3);
            CHECK((V_ac - V_ac_ref).norm() < 1e-10);
            grid.unset_changes();

            const CplxVect V_dc = grid.dc_pf(flat_start(grid), 1, 1e-8);
            REQUIRE(V_dc.size() == 3);
            CHECK((V_dc - V_dc_ref).norm() < 1e-10);
            grid.unset_changes();
        }
    }

    SECTION("the cached path still gives the same answer as a full rebuild") {
        // the reuse itself must stay lossless: same setpoint changes, with and
        // without the "nothing changed" claim in between.
        LSGrid cached = make_three_bus_grid();
        LSGrid rebuilt = make_three_bus_grid();
        for(int step = 0; step < 4; ++step){
            const real_type p = 50. + 5. * step;
            cached.change_p_load(0, p);
            rebuilt.change_p_load(0, p);

            const CplxVect V_cached = solve_ac(cached);
            const CplxVect V_rebuilt = solve_ac(rebuilt);
            REQUIRE(V_cached.size() == 3);
            REQUIRE(V_rebuilt.size() == 3);
            CHECK((V_cached - V_rebuilt).norm() < 1e-10);
