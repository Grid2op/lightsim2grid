...
void SolverRegistry::register_solver(const std::string& name, Factory f) {
    _factories[name] = std::move(f);
}

std::unique_ptr<BaseAlgo> SolverRegistry::make(const std::string& name) const {
    auto it = _factories.find(name);
    if (it == _factories.end()) {
        throw std::runtime_error("SolverRegistry: unknown solver '" + name + "'. "
                                 "Check available_solver_names() for the list of registered solvers.");
    }
    return it->second();
}

bool SolverRegistry::is_registered(const std::string& name) const {
