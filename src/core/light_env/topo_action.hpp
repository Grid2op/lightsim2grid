    ...
    public:
        ...
        void apply_to_gridmodel(LSGrid & grid) const {
            update_grid_values(grid,
                               loads_bus,
                               &LSGrid::change_bus_load,
                               &LSGrid::deactivate_load,
                               &LSGrid::reactivate_load);
            update_grid_values(grid,
                               gens_bus,
                               &LSGrid::change_bus_gen,
                               &LSGrid::deactivate_gen,
                               &LSGrid::reactivate_gen);
            update_grid_values(grid,
                               lines_or_bus,
                               &LSGrid::change_bus1_powerline,
                               &LSGrid::deactivate_powerline,
                               &LSGrid::reactivate_powerline);
            update_grid_values(grid,
                               lines_ex_bus,
                               &LSGrid::change_bus2_powerline,
                               &LSGrid::deactivate_powerline,
                               &LSGrid::reactivate_powerline);
            update_grid_values(grid,
                               trafo_hv_bus,
                               &LSGrid::change_bus1_trafo,
                               &LSGrid::deactivate_trafo,
                               &LSGrid::reactivate_trafo);
            update_grid_values(grid,
                               trafo_lv_bus,
                               &LSGrid::change_bus2_trafo,
                               &LSGrid::deactivate_trafo,
                               &LSGrid::reactivate_trafo);
            update_grid_values(grid,
                               storages_bus,
                               &LSGrid::change_bus_storage,
                               &LSGrid::deactivate_storage,
                               &LSGrid::reactivate_storage);
                                ...
                                FunctorTypeReco fun_reco) const
        {
            for(const auto & el : new_values)
            {
                ...
                if(bus_global_id == -1)
                {
                    ...
                    (grid.*fun_reco)(el_id);
                    (grid.*fun_change)(el_id, GridModelBusId(bus_global_id));
                }
            }
        }
