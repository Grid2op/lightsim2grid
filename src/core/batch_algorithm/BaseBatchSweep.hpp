...
class LS2G_API BaseBatchSweep: public BaseBatchSolverSynch
{
    public:
        ...
        // path runs the rows with -- so at nb_thread == 1 keeping the base case already
        // means keeping the factorization. At nb_thread > 1 the rows are run by workers
        // instead, and those were rebuilt from scratch every call: a fresh
        // AlgorithmSelector each, and a first row paying build_J_sparsity + analyze +
        // factorize rather than a refactorize. This keeps them, so a threaded batch gets
        // the same treatment its single-threaded twin already had.
        //
        // The workers are L2 state, like the member algorithm: anything that drops the
        // batch inputs drops them too (see clear_batch_inputs), which is what makes it
        // safe to keep something holding a ledger, a sparsity and a factorization.
        void set_reuse_thread_algos(bool val) {
            if(val == _reuse_thread_algos_) return;
            _reuse_thread_algos_ = val;
            if(!val) clear_batch_inputs();
        }
        bool get_reuse_thread_algos() const { return _reuse_thread_algos_; }
    ...
    private:
        ...
        // part of L2, same as the member _algo (see clear_batch_inputs). Empty on the
        // single-threaded path, and whenever the batch inputs were dropped. Indexed by
