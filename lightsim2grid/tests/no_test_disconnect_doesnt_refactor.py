...
class TestRefactorNotUsed(unittest.TestCase):
    ...
    def test_disco_line_doesnt_refactor(self):
        ...
        self.gridmodel.ac_pf(self.v_init, self.iter, self.tol_solver)
        print("PF done")
        timers_jac = self.gridmodel.get_solver().get_timers_jacobian()
        print(f"{timers_jac.timer_initialize = }")
        print(f"{timers_jac.timer_factor + timers_jac.timer_refactor = }")
        print(f"{timers_jac.timer_solve = }")
