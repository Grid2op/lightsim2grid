...
class TestMultipleSlack14(unittest.TestCase):
    ...
    def check_results(self, V_ls, ls_grid, pp_net):
        ...
        assert np.all(np.abs([el.res_p1_mw for el in ls_grid.get_lines()] - pp_net.res_line["p_from_mw"].values) <= tol_line), f"{np.abs([el.res_p_or_mw for el in ls_grid.get_lines()] - pp_net.res_line['p_from_mw'].values).max()}"
        assert np.all(np.abs([el.res_p1_mw for el in ls_grid.get_trafos()] - pp_net.res_trafo["p_hv_mw"].values) <= tol_line)
