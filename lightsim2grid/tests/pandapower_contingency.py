...
# copied on August, 27th, 2024 from 
...
#############################
# way to import lightsim2grid modified to work correctly independantly
# of lightsim2grid version
# see https://github.com/e2nIEE/pandapower/issues/2361#issuecomment-2263091364
...
def run_contingency_ls2g(net, nminus1_cases, contingency_evaluation_function=pp.runpp, **kwargs):
    ...
    **contingency_evaluation_function** - func
    ...
    sa = ContingencyAnalysisCPP(lightsim_grid_model)
    sa.change_solver(solver_type)
    ...
    for element, values in nminus1_cases.items():
        ...
        if element == "trafo3w":
            ...
            index += n_lines
        sa.add_multiple_n1(index)

    # s.add_multiple_n1(net.line.index.values.astype(int))
    v_init = net._ppc["internal"]["V"]
    sa.compute(v_init, net._options["max_iteration"], net._options["tolerance_mva"])
    v_res = sa.get_voltages()
    sa.compute_flows()
    kamps_all = sa.get_flows()
    vm_pu = np.abs(v_res)
